import React, { useState, useEffect, useMemo } from "react";
import api from "./api";
import "./themeSwitch.css";
import "./loader.css"; // 👈 NEW

import {
  AppBar,
  Toolbar,
  Typography,
  Container,
  Tabs,
  Tab,
  Box,
  Grid,
  Paper,
  TextField,
  Button,
  Table,
  TableBody,
  TableCell,
  TableContainer,
  TableHead,
  TableRow,
  LinearProgress,
  Snackbar,
  Alert,
  FormControl,
  InputLabel,
  Select,
  MenuItem,
  Stack,
  CssBaseline
} from "@mui/material";
import { ThemeProvider, createTheme } from "@mui/material/styles";

// ---------- THEME FACTORY ----------
function getAppTheme(mode) {
  const isDark = mode === "dark";

  return createTheme({
    palette: {
      mode,
      background: {
        default: isDark ? "#000000" : "#f5f5f5",
        paper: isDark ? "#000000" : "#ffffff"
      },
      text: {
        primary: isDark ? "#ffffff" : "#111111",
        secondary: isDark ? "#b0b0b0" : "#555555"
      }
    },
    components: {
      MuiPaper: {
        styleOverrides: {
          root: {
            backgroundImage: "none"
          }
        }
      },
      MuiTableCell: {
        styleOverrides: {
          root: {
            borderColor: isDark ? "#333333" : "#e0e0e0"
          }
        }
      }
    }
  });
}

// ---------- LOADER ----------
function Loader() {
  return (
    <div>
      <svg
        height="108px"
        width="108px"
        viewBox="0 0 128 128"
        className="loader"
      >
        <defs>
          <clipPath id="loader-eyes">
            <circle
              transform="rotate(-40,64,64) translate(0,-56)"
              r="8"
              cy="64"
              cx="64"
              className="loader__eye1"
            ></circle>
            <circle
              transform="rotate(40,64,64) translate(0,-56)"
              r="8"
              cy="64"
              cx="64"
              className="loader__eye2"
            ></circle>
          </clipPath>
          <linearGradient y2="1" x2="0" y1="0" x1="0" id="loader-grad">
            <stop stopColor="#000" offset="0%"></stop>
            <stop stopColor="#fff" offset="100%"></stop>
          </linearGradient>
          <mask id="loader-mask">
            <rect
              fill="url(#loader-grad)"
              height="128"
              width="128"
              y="0"
              x="0"
            ></rect>
          </mask>
        </defs>
        <g strokeDasharray="175.93 351.86" strokeWidth="12" strokeLinecap="round">
          <g>
            <rect
              clipPath="url(#loader-eyes)"
              height="64"
              width="128"
              fill="hsl(193,90%,50%)"
            ></rect>
            <g stroke="hsl(193,90%,50%)" fill="none">
              <circle
                transform="rotate(180,64,64)"
                r="56"
                cy="64"
                cx="64"
                className="loader__mouth1"
              ></circle>
              <circle
                transform="rotate(0,64,64)"
                r="56"
                cy="64"
                cx="64"
                className="loader__mouth2"
              ></circle>
            </g>
          </g>
          <g mask="url(#loader-mask)">
            <rect
              clipPath="url(#loader-eyes)"
              height="64"
              width="128"
              fill="hsl(223,90%,50%)"
            ></rect>
            <g stroke="hsl(223,90%,50%)" fill="none">
              <circle
                transform="rotate(180,64,64)"
                r="56"
                cy="64"
                cx="64"
                className="loader__mouth1"
              ></circle>
              <circle
                transform="rotate(0,64,64)"
                r="56"
                cy="64"
                cx="64"
                className="loader__mouth2"
              ></circle>
            </g>
          </g>
        </g>
      </svg>
    </div>
  );
}

function AppInner() {
  const [activeTab, setActiveTab] = useState("dashboard");

  const [year, setYear] = useState(2025);
  const [month, setMonth] = useState(11);

  // Salesperson & brand / section mapping
  const [spName, setSpName] = useState("");
  const [spBrand, setSpBrand] = useState("");
  const [spSection, setSpSection] = useState("");
  const [salespersons, setSalespersons] = useState([]);
  const [brandTable, setBrandTable] = useState([]);

  // Targets (bulk)
  const [targetsTable, setTargetsTable] = useState([]);

  // Upload (sales and sale returns)
  const [salesFile, setSalesFile] = useState(null);
  const [returnsFile, setReturnsFile] = useState(null);
  const [uploadStatus, setUploadStatus] = useState("");
  const [uploadLoading, setUploadLoading] = useState(false); // 👈 NEW

  // Dashboard
  const [dashboardData, setDashboardData] = useState([]);
  const [loadingDashboard, setLoadingDashboard] = useState(false);

  // Dashboard filters
  const [brandFilter, setBrandFilter] = useState("ALL");
  const [sectionFilter, setSectionFilter] = useState("ALL");

  // Snackbar
  const [snack, setSnack] = useState({
    open: false,
    message: "",
    severity: "info"
  });

  const handleSnackClose = () => {
    setSnack((prev) => ({ ...prev, open: false }));
  };

  // Load salespersons once
  useEffect(() => {
    fetchSalespersons();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const fetchSalespersons = async () => {
    try {
      const res = await api.get("/api/salespersons");
      const list = res.data || [];
      setSalespersons(list);
      setBrandTable(
        list.map((sp) => ({
          id: sp._id,
          name: sp.name,
          brand: sp.brand || "",
          section: sp.section || ""
        }))
      );
    } catch (err) {
      console.error(err);
      setSnack({
        open: true,
        message: "Failed to load salespersons",
        severity: "error"
      });
    }
  };

  const fetchDashboard = async () => {
    try {
      setLoadingDashboard(true);
      const res = await api.get("/api/dashboard", {
        params: { year, month }
      });
      const data = res.data || [];
      setDashboardData(data);

      // Initialise targets editing table from dashboard data
      setTargetsTable(
        data.map((row) => ({
          name: row.name,
          brand: row.brand,
          section: row.section,
          target: row.target || 0
        }))
      );
    } catch (err) {
      console.error(err);
      setSnack({
        open: true,
        message: "Error loading dashboard",
        severity: "error"
      });
    } finally {
      setLoadingDashboard(false);
    }
  };

  useEffect(() => {
    fetchDashboard();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleAddSalesperson = async (e) => {
    e.preventDefault();
    if (!spName.trim()) {
      setSnack({
        open: true,
        message: "Salesperson name is required",
        severity: "warning"
      });
      return;
    }
    try {
      await api.post("/api/salespersons", {
        name: spName,
        brand: spBrand || null,
        section: spSection || null
      });
      setSpName("");
      setSpBrand("");
      setSpSection("");
      await fetchSalespersons();
      await fetchDashboard();
      setSnack({
        open: true,
        message: "Salesperson saved",
        severity: "success"
      });
    } catch (err) {
      console.error(err);
      setSnack({
        open: true,
        message: "Error saving salesperson",
        severity: "error"
      });
    }
  };

  // Bulk save brand + section
  const handleSalespersonFieldChange = (index, field, value) => {
    setBrandTable((prev) => {
      const copy = [...prev];
      copy[index] = { ...copy[index], [field]: value };
      return copy;
    });
  };

  const handleSaveAllBrands = async () => {
    try {
      await api.post("/api/salespersons/bulk", {
        salespersons: brandTable.map((row) => ({
          id: row.id,
          brand: row.brand,
          section: row.section
        }))
      });
      await fetchSalespersons();
      await fetchDashboard();
      setSnack({
        open: true,
        message: "All brands / sections updated",
        severity: "success"
      });
    } catch (err) {
      console.error(err);
      setSnack({
        open: true,
        message: "Error updating brands / sections",
        severity: "error"
      });
    }
  };

  // Bulk save targets
  const handleTargetChange = (index, value) => {
    setTargetsTable((prev) => {
      const copy = [...prev];
      copy[index] = { ...copy[index], target: value };
      return copy;
    });
  };

  const handleSaveAllTargets = async () => {
    try {
      await api.post("/api/targets/bulk", {
        year,
        month,
        targets: targetsTable.map((row) => ({
          name: row.name,
          target: Number(row.target) || 0
        }))
      });
      await fetchDashboard();
      setSnack({
        open: true,
        message: "Targets updated for all salespersons",
        severity: "success"
      });
    } catch (err) {
      console.error(err);
      setSnack({
        open: true,
        message: "Error updating targets",
        severity: "error"
      });
    }
  };

  const handleUploadSales = async (e) => {
    e.preventDefault();

    // At least one file should be provided: sales or returns
    if (!salesFile && !returnsFile) {
      setSnack({
        open: true,
        message: "Please select at least a Sales Excel file",
        severity: "warning"
      });
      return;
    }

    try {
      setUploadLoading(true); // 👈 NEW
      setUploadStatus("Uploading and processing...");
      const formData = new FormData();

      // Backend expects salesFile and returnsFile
      if (salesFile) {
        formData.append("salesFile", salesFile);
      }
      if (returnsFile) {
        formData.append("returnsFile", returnsFile);
      }

      // For backward compatibility (if server still checks 'file')
      if (salesFile) {
        formData.append("file", salesFile);
      }

      formData.append("year", year);
      formData.append("month", month);

      await api.post("/api/upload-sales", formData, {
        headers: {
          "Content-Type": "multipart/form-data"
        }
      });

      setUploadStatus("Sales data processed successfully");
      setSalesFile(null);
      setReturnsFile(null);
      await fetchDashboard();
      setSnack({
        open: true,
        message: "Sales and sale return files processed",
        severity: "success"
      });
    } catch (err) {
      console.error(err);
      setUploadStatus("Error processing file(s)");
      setSnack({
        open: true,
        message: "Error processing file(s)",
        severity: "error"
      });
    } finally {
      setUploadLoading(false); // 👈 NEW
    }
  };

  const formatCurrency = (val) => {
    if (val == null) return "0";
    return val.toLocaleString("en-IN", {
      maximumFractionDigits: 0
    });
  };

  const formatPercent = (val) => {
    if (val == null) return "-";
    return val.toFixed(1) + "%";
  };

  // ---- Filters ----
  const brandOptions = useMemo(
    () =>
      Array.from(
        new Set(
          dashboardData
            .map((r) => r.brand)
            .filter((b) => b && String(b).trim() !== "")
        )
      ).sort(),
    [dashboardData]
  );

  const sectionOptions = useMemo(
    () =>
      Array.from(
        new Set(
          dashboardData
            .map((r) => r.section)
            .filter((s) => s && String(s).trim() !== "")
        )
      ).sort(),
    [dashboardData]
  );

  const filteredDashboardData = dashboardData.filter((row) => {
    const brandOk =
      brandFilter === "ALL" ||
      (row.brand || "").toUpperCase() === brandFilter.toUpperCase();
    const sectionOk =
      sectionFilter === "ALL" ||
      (row.section || "").toUpperCase() === sectionFilter.toUpperCase();
    return brandOk && sectionOk;
  });

  // Summary over filtered rows
  const totalTargetAll = filteredDashboardData.reduce(
    (sum, r) => sum + (r.target || 0),
    0
  );
  const totalAchAll = filteredDashboardData.reduce(
    (sum, r) => sum + (r.totalAchievement || 0),
    0
  );
  const overallPercent =
    totalTargetAll > 0 ? (totalAchAll / totalTargetAll) * 100 : null;

  const MONTHS = [
    { value: 1, label: "January" },
    { value: 2, label: "February" },
    { value: 3, label: "March" },
    { value: 4, label: "April" },
    { value: 5, label: "May" },
    { value: 6, label: "June" },
    { value: 7, label: "July" },
    { value: 8, label: "August" },
    { value: 9, label: "September" },
    { value: 10, label: "October" },
    { value: 11, label: "November" },
    { value: 12, label: "December" }
  ];

  return (
    <>
      <AppBar position="static">
        <Toolbar sx={{ display: "flex", justifyContent: "space-between" }}>
          <Typography variant="h6">Sales Target Dashboard</Typography>
          {/* Theme toggle lives in parent, fixed box */}
        </Toolbar>
      </AppBar>

      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        {/* Period selector */}
        <Paper sx={{ p: 2, mb: 3 }}>
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={3}>
              <Typography variant="subtitle1" fontWeight={600}>
                Period
              </Typography>
            </Grid>
            <Grid item xs={12} md={3}>
              <TextField
                label="Year"
                type="number"
                fullWidth
                value={year}
                onChange={(e) => setYear(Number(e.target.value))}
              />
            </Grid>
            <Grid item xs={12} md={3}>
              <FormControl fullWidth>
                <InputLabel>Month</InputLabel>
                <Select
                  value={month}
                  label="Month"
                  onChange={(e) => setMonth(Number(e.target.value))}
                >
                  {MONTHS.map((m) => (
                    <MenuItem key={m.value} value={m.value}>
                      {m.label}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12} md={3}>
              <Button
                variant="contained"
                fullWidth
                onClick={fetchDashboard}
                sx={{ height: { md: "100%", xs: "auto" } }}
              >
                Refresh Dashboard
              </Button>
            </Grid>
          </Grid>
        </Paper>

        {/* Tabs */}
        <Paper sx={{ mb: 3 }}>
          <Tabs
            value={activeTab}
            onChange={(_, val) => setActiveTab(val)}
            variant="scrollable"
            scrollButtons="auto"
          >
            <Tab label="Dashboard" value="dashboard" />
            <Tab label="Set Targets" value="targets" />
            <Tab label="Upload Sales / Returns" value="upload" />
            <Tab
              label="Salespersons / Brand & Section"
              value="salespersons"
            />
          </Tabs>
        </Paper>

        {/* Tab content */}
        {activeTab === "dashboard" && (
          <Box>
            {/* Filters */}
            <Paper sx={{ p: 2, mb: 3 }}>
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth size="small">
                    <InputLabel>Filter by Brand</InputLabel>
                    <Select
                      value={brandFilter}
                      label="Filter by Brand"
                      onChange={(e) => setBrandFilter(e.target.value)}
                    >
                      <MenuItem value="ALL">All Brands</MenuItem>
                      {brandOptions.map((b) => (
                        <MenuItem key={b} value={b}>
                          {b}
                        </MenuItem>
                      ))}
                    </Select>
                  </FormControl>
                </Grid>
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth size="small">
                    <InputLabel>Filter by Section</InputLabel>
                    <Select
                      value={sectionFilter}
                      label="Filter by Section"
                      onChange={(e) => setSectionFilter(e.target.value)}
                    >
                      <MenuItem value="ALL">All Sections</MenuItem>
                      {sectionOptions.map((s) => (
                        <MenuItem key={s} value={s}>
                          {s}
                        </MenuItem>
                      ))}
                    </Select>
                  </FormControl>
                </Grid>
              </Grid>
            </Paper>

            {/* Summary cards */}
            <Grid container spacing={3} sx={{ mb: 3 }}>
              <Grid item xs={12} md={4}>
                <Paper sx={{ p: 2 }}>
                  <Typography variant="subtitle2" color="text.secondary">
                    Total Target
                  </Typography>
                  <Typography variant="h6">
                    ₹ {formatCurrency(totalTargetAll)}
                  </Typography>
                </Paper>
              </Grid>
              <Grid item xs={12} md={4}>
                <Paper sx={{ p: 2 }}>
                  <Typography variant="subtitle2" color="text.secondary">
                    Total Achievement
                  </Typography>
                  <Typography variant="h6">
                    ₹ {formatCurrency(totalAchAll)}
                  </Typography>
                </Paper>
              </Grid>
              <Grid item xs={12} md={4}>
                <Paper sx={{ p: 2 }}>
                  <Typography variant="subtitle2" color="text.secondary">
                    Overall Completion
                  </Typography>
                  <Typography variant="h6">
                    {formatPercent(overallPercent)}
                  </Typography>
                  <LinearProgress
                    variant="determinate"
                    value={overallPercent ? Math.min(overallPercent, 100) : 0}
                    sx={{ mt: 1.5 }}
                  />
                </Paper>
              </Grid>
            </Grid>

            <Paper sx={{ p: 2 }}>
              {loadingDashboard && (
                <Box sx={{ mb: 2 }}>
                  <LinearProgress />
                </Box>
              )}
              <TableContainer>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      <TableCell>Salesperson</TableCell>
                      <TableCell>Brand</TableCell>
                      <TableCell>Section</TableCell>
                      <TableCell align="right">Target (₹)</TableCell>
                      <TableCell align="right">Own Ach (₹)</TableCell>
                      <TableCell align="right">Other Ach (₹)</TableCell>
                      <TableCell align="right">Total Ach (₹)</TableCell>
                      <TableCell align="right">Completion</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {filteredDashboardData.map((row) => {
                      const pct = row.totalPercent || 0;
                      const barValue = pct ? Math.min(pct, 100) : 0;

                      return (
                        <TableRow key={row.name}>
                          <TableCell>{row.name}</TableCell>
                          <TableCell>{row.brand || "-"}</TableCell>
                          <TableCell>{row.section || "-"}</TableCell>
                          <TableCell align="right">
                            ₹ {formatCurrency(row.target || 0)}
                          </TableCell>
                          <TableCell align="right">
                            ₹ {formatCurrency(row.ownAchievement || 0)}
                          </TableCell>
                          <TableCell align="right">
                            ₹ {formatCurrency(row.otherAchievement || 0)}
                          </TableCell>
                          <TableCell align="right">
                            ₹ {formatCurrency(row.totalAchievement || 0)}
                          </TableCell>
                          <TableCell align="right" sx={{ minWidth: 140 }}>
                            <Stack spacing={0.5}>
                              <Typography variant="body2">
                                {formatPercent(row.totalPercent)}
                              </Typography>
                              <LinearProgress
                                variant="determinate"
                                value={barValue}
                              />
                            </Stack>
                          </TableCell>
                        </TableRow>
                      );
                    })}
                    {filteredDashboardData.length === 0 && !loadingDashboard && (
                      <TableRow>
                        <TableCell colSpan={8} align="center">
                          No data for this month / filters
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
              </TableContainer>
            </Paper>
          </Box>
        )}

        {activeTab === "targets" && (
          <Box>
            <Paper sx={{ p: 2 }}>
              <Typography variant="h6" sx={{ mb: 1 }}>
                Set Monthly Targets (Bulk)
              </Typography>
              <Typography
                variant="body2"
                color="text.secondary"
                sx={{ mb: 2 }}
              >
                Edit targets for all salespersons for {month}/{year} and click
                “Save All Targets”.
              </Typography>
              <TableContainer>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      <TableCell>Salesperson</TableCell>
                      <TableCell>Brand</TableCell>
                      <TableCell>Section</TableCell>
                      <TableCell>Target (₹)</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {targetsTable.map((row, index) => (
                      <TableRow key={row.name}>
                        <TableCell>{row.name}</TableCell>
                        <TableCell>{row.brand || "-"}</TableCell>
                        <TableCell>{row.section || "-"}</TableCell>
                        <TableCell sx={{ maxWidth: 140 }}>
                          <TextField
                            type="number"
                            size="small"
                            value={row.target}
                            onChange={(e) =>
                              handleTargetChange(index, e.target.value)
                            }
                          />
                        </TableCell>
                      </TableRow>
                    ))}
                    {targetsTable.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={4} align="center">
                          No salespersons found for targets
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
              </TableContainer>
              <Box sx={{ mt: 2, textAlign: "right" }}>
                <Button variant="contained" onClick={handleSaveAllTargets}>
                  Save All Targets
                </Button>
              </Box>
            </Paper>
          </Box>
        )}

        {activeTab === "upload" && (
          <Box>
            <Paper sx={{ p: 2 }}>
              <Typography variant="h6" sx={{ mb: 2 }}>
                Upload Sales and Sale Return Excel (All Salespersons)
              </Typography>

              <Box
                component="form"
                onSubmit={handleUploadSales}
                sx={{ display: "flex", flexDirection: "column", gap: 2 }}
              >
                <Button variant="outlined" component="label">
                  {salesFile ? salesFile.name : "Choose Sales Excel file"}
                  <input
                    type="file"
                    accept=".xls,.xlsx"
                    hidden
                    onChange={(e) => setSalesFile(e.target.files[0] || null)}
                  />
                </Button>

                <Button variant="outlined" component="label">
                  {returnsFile
                    ? returnsFile.name
                    : "Choose Sale Return Excel file (optional)"}
                  <input
                    type="file"
                    accept=".xls,.xlsx"
                    hidden
                    onChange={(e) => setReturnsFile(e.target.files[0] || null)}
                  />
                </Button>

                <Button type="submit" variant="contained">
                  Upload and Process
                </Button>
              </Box>

              {uploadStatus && (
                <Typography variant="body2" sx={{ mt: 2 }}>
                  {uploadStatus}
                </Typography>
              )}
              <Typography
                variant="body2"
                color="text.secondary"
                sx={{ mt: 2 }}
              >
                Upload one Sales Excel file that contains all salespersons and,
                if available, a Sale Return Excel file for the same period. The
                backend will read each <code>Salesperson : NAME</code> block,
                apply all business rules, and subtract sale returns from the
                achievements.
              </Typography>
            </Paper>
          </Box>
        )}

        {activeTab === "salespersons" && (
          <Box>
            <Paper sx={{ p: 2, mb: 3 }}>
              <Typography variant="h6" sx={{ mb: 2 }}>
                Add New Salesperson
              </Typography>
              <Box
                component="form"
                onSubmit={handleAddSalesperson}
                sx={{
                  display: "flex",
                  flexDirection: { xs: "column", md: "row" },
                  gap: 2
                }}
              >
                <TextField
                  label="New salesperson name"
                  fullWidth
                  value={spName}
                  onChange={(e) => setSpName(e.target.value)}
                  placeholder="Example: SATHIYA"
                />
                <TextField
                  label="Brand (optional)"
                  fullWidth
                  value={spBrand}
                  onChange={(e) => setSpBrand(e.target.value)}
                  placeholder="Example: VIDEUM"
                />
                <TextField
                  label="Section (optional)"
                  fullWidth
                  value={spSection}
                  onChange={(e) => setSpSection(e.target.value)}
                  placeholder="Example: AC / TV / SMALL APPL"
                />
                <Button
                  type="submit"
                  variant="contained"
                  sx={{ minWidth: 160 }}
                >
                  Add Salesperson
                </Button>
              </Box>
            </Paper>

            <Paper sx={{ p: 2 }}>
              <Typography variant="h6" sx={{ mb: 1 }}>
                Salespersons, Brands and Sections
              </Typography>
              <Typography
                variant="body2"
                color="text.secondary"
                sx={{ mb: 2 }}
              >
                Edit Brand and Section directly in the table below and click
                “Save All Brands / Sections”.
              </Typography>

              <TableContainer>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      <TableCell>Salesperson</TableCell>
                      <TableCell>Brand</TableCell>
                      <TableCell>Section</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {brandTable.map((row, index) => (
                      <TableRow key={row.id}>
                        <TableCell>{row.name}</TableCell>
                        <TableCell sx={{ maxWidth: 180 }}>
                          <TextField
                            size="small"
                            value={row.brand}
                            onChange={(e) =>
                              handleSalespersonFieldChange(
                                index,
                                "brand",
                                e.target.value
                              )
                            }
                            placeholder="-"
                            fullWidth
                          />
                        </TableCell>
                        <TableCell sx={{ maxWidth: 180 }}>
                          <TextField
                            size="small"
                            value={row.section}
                            onChange={(e) =>
                              handleSalespersonFieldChange(
                                index,
                                "section",
                                e.target.value
                              )
                            }
                            placeholder="-"
                            fullWidth
                          />
                        </TableCell>
                      </TableRow>
                    ))}
                    {brandTable.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={3} align="center">
                          No salespersons yet
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
              </TableContainer>

              <Box sx={{ mt: 2, textAlign: "right" }}>
                <Button variant="contained" onClick={handleSaveAllBrands}>
                  Save All Brands / Sections
                </Button>
              </Box>
            </Paper>
          </Box>
        )}
      </Container>

      {/* Snackbar for messages */}
      <Snackbar
        open={snack.open}
        autoHideDuration={3000}
        onClose={handleSnackClose}
        anchorOrigin={{ vertical: "bottom", horizontal: "right" }}
      >
        <Alert
          onClose={handleSnackClose}
          severity={snack.severity}
          variant="filled"
          sx={{ width: "100%" }}
        >
          {snack.message}
        </Alert>
      </Snackbar>

      {/* FULL-SCREEN LOADER OVERLAY */}
      {(loadingDashboard || uploadLoading) && (
        <Box
          sx={{
            position: "fixed",
            inset: 0,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            bgcolor: "rgba(0,0,0,0.4)",
            zIndex: 3000
          }}
        >
          <Loader />
        </Box>
      )}
    </>
  );
}

function ThemeToggle({ mode, onToggle }) {
  const checked = mode === "dark";

  return (
    <label className="theme-switch">
      <input
        type="checkbox"
        className="theme-switch__checkbox"
        checked={checked}
        onChange={onToggle}
      />
      <div className="theme-switch__container">
        <div className="theme-switch__clouds"></div>
        <div className="theme-switch__stars-container">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 144 55"
            fill="none"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M135.831 3.00688C135.055 3.85027 134.111 4.29946 133 4.35447C134.111 4.40947 135.055 4.85867 135.831 5.71123C136.607 6.55462 136.996 7.56303 136.996 8.72727C136.996 7.95722 137.172 7.25134 137.525 6.59129C137.886 5.93124 138.372 5.39954 138.98 5.00535C139.598 4.60199 140.268 4.39114 141 4.35447C139.88 4.2903 138.936 3.85027 138.16 3.00688C137.384 2.16348 136.996 1.16425 136.996 0C136.996 1.16425 136.607 2.16348 135.831 3.00688ZM31 23.3545C32.1114 23.2995 33.0551 22.8503 33.8313 22.0069C34.6075 21.1635 34.9956 20.1642 34.9956 19C34.9956 20.1642 35.3837 21.1635 36.1599 22.0069C36.9361 22.8503 37.8798 23.2903 39 23.3545C38.2679 23.3911 37.5976 23.602 36.9802 24.0053C36.3716 24.3995 35.8864 24.9312 35.5248 25.5913C35.172 26.2513 34.9956 26.9572 34.9956 27.7273C34.9956 26.563 34.6075 25.5546 33.8313 24.7112C33.0551 23.8587 32.1114 23.4095 31 23.3545ZM0 36.3545C1.11136 36.2995 2.05513 35.8503 2.83131 35.0069C3.6075 34.1635 3.99559 33.1642 3.99559 32C3.99559 33.1642 4.38368 34.1635 5.15987 35.0069C5.93605 35.8503 6.87982 36.2903 8 36.3545C7.26792 36.3911 6.59757 36.602 5.98015 37.0053C5.37155 37.3995 4.88644 37.9312 4.52481 38.5913C4.172 39.2513 3.99559 39.9572 3.99559 40.7273C3.99559 39.563 3.6075 38.5546 2.83131 37.7112C2.05513 36.8587 1.11136 36.4095 0 36.3545ZM56.8313 24.0069C56.0551 24.8503 55.1114 25.2995 54 25.3545C55.1114 25.4095 56.0551 25.8587 56.8313 26.7112C57.6075 27.5546 57.9956 28.563 57.9956 29.7273C57.9956 28.9572 58.172 28.2513 58.5248 27.5913C58.8864 26.9312 59.3716 26.3995 59.9802 26.0053C60.5976 25.602 61.2679 25.3911 62 25.3545C60.8798 25.2903 59.9361 24.8503 59.1599 24.0069C58.3837 23.1635 57.9956 22.1642 57.9956 21C57.9956 22.1642 57.6075 23.1635 56.8313 24.0069ZM81 25.3545C82.1114 25.2995 83.0551 24.8503 83.8313 24.0069C84.6075 23.1635 84.9956 22.1642 84.9956 21C84.9956 22.1642 85.3837 23.1635 86.1599 24.0069C86.9361 24.8503 87.8798 25.2903 89 25.3545C88.2679 25.3911 87.5976 25.602 86.9802 26.0053C86.3716 26.3995 85.8864 26.9312 85.5248 27.5913C85.172 28.2513 84.9956 28.9572 84.9956 29.7273C84.9956 28.563 84.6075 27.5546 83.8313 26.7112C83.0551 25.8587 82.1114 25.4095 81 25.3545ZM136 36.3545C137.111 36.2995 138.055 35.8503 138.831 35.0069C139.607 34.1635 139.996 33.1642 139.996 32C139.996 33.1642 140.384 34.1635 141.16 35.0069C141.936 35.8503 142.88 36.2903 144 36.3545C143.268 36.3911 142.598 36.602 141.98 37.0053C141.372 37.3995 140.886 37.9312 140.525 38.5913C140.172 39.2513 139.996 39.9572 139.996 40.7273C139.996 39.563 139.607 38.5546 138.831 37.7112C138.055 36.8587 137.111 36.4095 136 36.3545ZM101.831 49.0069C101.055 49.8503 100.111 50.2995 99 50.3545C100.111 50.4095 101.055 50.8587 101.831 51.7112C102.607 52.5546 102.996 53.563 102.996 54.7273C102.996 53.9572 103.172 53.2513 103.525 52.5913C103.886 51.9312 104.372 51.3995 104.98 51.0053C105.598 50.602 106.268 50.3911 107 50.3545C105.88 50.2903 104.936 49.8503 104.16 49.0069C103.384 48.1635 102.996 47.1642 102.996 46C102.996 47.1642 102.607 48.1635 101.831 49.0069Z"
              fill="currentColor"
            />
          </svg>
        </div>
        <div className="theme-switch__circle-container">
          <div className="theme-switch__sun-moon-container">
            <div className="theme-switch__moon">
              <div className="theme-switch__spot"></div>
              <div className="theme-switch__spot"></div>
              <div className="theme-switch__spot"></div>
            </div>
          </div>
        </div>
      </div>
    </label>
  );
}

// ---------- WRAPPER WITH THEME TOGGLE ----------
function App() {
  const [mode, setMode] = useState("dark"); // default dark

  const theme = useMemo(() => getAppTheme(mode), [mode]);

  const toggleMode = () => {
    setMode((prev) => (prev === "dark" ? "light" : "dark"));
  };

  return (
    <ThemeProvider theme={theme}>
      <CssBaseline />
      {/* Floating toggle in the corner */}
      <Box sx={{ position: "fixed", top: 8, right: 8, zIndex: 2000 }}>
        <ThemeToggle mode={mode} onToggle={toggleMode} />
      </Box>
      <AppInner />
    </ThemeProvider>
  );
}

export default App;
